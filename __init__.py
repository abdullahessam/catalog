...
def showLogin():
    if request.method == 'POST':
        ...
        if user:
            ...
            return "wrong password or username please try again"
    else:
...
def signup():
    ...
    user = User(name=username, email=email, picture='', password=password)
    session.add(user)
    session.commit()
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant != login_session['user_id']:
        flash('you are not authorized to edit this resturant')
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != creator.id:
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
        if restaurantToDelete.user_id == creator.id:
...
def editMenuItem(restaurant_id, menu_id):
    ...
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    if editedItem != login_session['user_id']:
        flash('you are not authorized to edit this resturant')
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
        creator = getUserInfo(restaurant.user_id)
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if itemToDelete != login_session['user_id']:
        flash('you are not authorized to edit this resturant')
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
        creator = getUserInfo(restaurant.user_id)
